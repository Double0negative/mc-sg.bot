    ...
    public void execute(Chat chat, User sender, String[] args) throws Exception {
        if (rate == null) {
            ...
            rate.start(); // Limit to 10 per 2 minutes to make sure we dont
                          // overlap minutes and get limited
        }
        if (count > 8) {
            chat.sendMessage("Weather rate limit exceeded. Try again in a couple minutes");
            return;
        }
        ...
        if (conditions == null || conditions.response == null) {
            ...
            if (day == null && hour == null) {
                ...
                String weather = StringUtils.replaceVars(CONDITIONS_FORMAT, cur.display_location.full, cur.temp_f,
                        cur.temp_c, cur.weather, cur.wind_dir, cur.wind_mph, cur.wind_kph, cur.wind_gust_mph,
                        cur.wind_gust_kph, cur.relative_humidity, cur.feelslike_f, cur.feelslike_c);
                ChatManager.chat(chat, weather);
            } else {
                CurrentObservation cur = conditions.current_observation;
                String weather = StringUtils.replaceVars(CONDITIONS_SHORT, cur.display_location.full, cur.temp_f,
                        cur.temp_c, cur.weather, cur.wind_dir, cur.wind_mph, cur.wind_kph, cur.relative_humidity);
                ChatManager.chat(chat, weather);

                int i = (swi.containsKey("number")) ? Integer.parseInt(swi.get("number"))
                        : Settings.Root.Weather.max_forecast;

                if (hour != null) {
                    for (int a = 0; a < i; a++) {
                        HourlyForecast fcst = hour.hourly_forecast[a];
                        String hourw = StringUtils.replaceVars(HOURLY_FORMAT, fcst.FCTTIME.civil, fcst.wx,
                                fcst.temp.english, fcst.temp.metric, fcst.wspd.english, fcst.wspd.metric,
                                fcst.wdir.dir, fcst.pop);
            ...
            if (alerts != null) {
                for (Alert alert : alerts.alerts) {
                    ChatManager.chat(chat, sender,
                            StringUtils.replaceVars(ALERTS_FORMAT, alert.description, alert.expires));
                }
            }
        }
