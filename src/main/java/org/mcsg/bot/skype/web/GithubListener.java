            ...
            public void run() {
                ...
                while (true) {
                    try {
                        ...
                        skt.close();
                        sendMessageToChats(headers.get("X-GitHub-Event").getValue(),
                                gson.fromJson(sb.toString(), GitHubMessage.class));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
        ...
        for (String chatid : chats) {
            ...
            if (type.equalsIgnoreCase("push")) {
                if (data.commits.size() > 0) {
                    StringBuilder sb = new StringBuilder();
                    sb.append(data.pusher.name + " pushed " + data.commits.size() + " new commit to "
                    ...
                    chat.sendMessage(sb.toString().trim());
                }
            }
        }
    }
